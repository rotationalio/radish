...
type Config struct {
	...
	BookkeepingTimeout time.Duration        `default:"5s" split_words:"true" desc:"The amount of time to allow task bookkeeping operations to complete after worker execution."`
...
func (c Config) Validate() (err error) {
	...
	if c.BookkeepingTimeout <= 0 {
		err = confire.Join(err, confire.Required("radish", "bookkeeping_timeout"))
	}
