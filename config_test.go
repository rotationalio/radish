...
func TestConfig(t *testing.T) {
	...
	t.Run("Invalid", func(t *testing.T) {
		...
		errs := map[string]string{
			...
			"RADISH_CLEANUP_TIMEOUT": "invalid configuration: radish.cleanup_timeout is required but not set",
	...
	// DefaultCleanupTimeout verifies that omitting the environment variable
	// applies the documented five-second default.
	t.Run("DefaultCleanupTimeout", func(t *testing.T) {
		t.Cleanup(mockEnv.Set())

		conf, err := LoadConfig()
		require.NoError(t, err, "could not process config from environment")
		require.Equal(t, 5*time.Second, conf.CleanupTimeout)
	})
}
